...
<head>
  ...
  <title>
    Pengadaan Alat petugas | Layanan Pengaduan Kominfo
  </title>
  <!--     Fonts and icons     -->
...
<body class="g-sidenav-show   bg-gray-100">
  ...
  <aside class="sidenav bg-white navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-4 " id="sidenav-main">
    ...
    <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link active" href="../pages/petugas_data_alat_pengadaan">
            <div class="icon icon-shape icon-sm border-radius-md text-center me-2 d-flex align-items-center justify-content-center">
  ...
  <main class="main-content position-relative border-radius-lg ">
      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-white active" aria-current="page">Pengadaan Alat</li>
          </ol>
          <h6 class="font-weight-bolder text-white mb-0">Pengadaan Alat</h6>
        </nav>
    ...
    <!-- End Navbar -->
    <!-- Modal -->
    <div class="modal fade" id="modalAlatPengadaan" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Tambah Data Pengadaan Alat</h5>
          ...
          <form id="tambahAlatPengadaan" action="petugas/alat_tambah/alat_tambah.php">
            <div class="modal-body">
              <!-- Isi form untuk menambah data -->
              <input type="text" class="d-none" name="status" id="status" value="Diproses">
    ...
    <!--Edit Modal -->
    <div class="modal fade" id="editModalAlatPengadaan" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Edit Data Pengadaan Alat</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close" style=" border: none; background-color: #ffffff; font-size: 35px;">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <form id="editAlatPengadaan" action="petugas/alat_tambah/alat_edit.php">
            <div class="modal-body">
              <!-- Isi form untuk menambah data -->
              <input type="text" class="d-none" name="id_alat_tambah" id="id_alat_tambah">
              <div class="form-group">
                <label for="id_alat">Nama Alat:</label>
        ...
        <div class="col-12">
          <div class="card mb-4">
            <div class="card-header pb-0">
              <h3 class="text-center">Data Pengadaan Alat</h3>
              ...
              <div class="dropdown">
                ...
                <!-- Tombol tambah -->
                <button class="btn btn-primary font-weight-bold text-xs" data-toggle="modal" data-target="#modalAlatPengadaan">
                  TAMBAH
                </button>
                <!-- Tombol alat_export -->
                <a href="petugas/alat_tambah/alat_export" target="_blank" class="btn btn-secondary font-weight-bold text-xs" data-toggle="tooltip" data-original-title="Edit user">
                  EXPORT
                </a>
              </div>
              ...
              <div class="table-responsive p-0">
                <table class="table align-items-center mb-0">
                  ...
                  <tbody>
                    ...
                    // Query SQL untuk mengambil data dari tabel alat_tambah, melakukan JOIN dengan tabel pengaduan untuk mendapatkan nama_dinas dan jenis_pengaduan, dan JOIN dengan tabel alat untuk mendapatkan nama alat
                    $query = "SELECT at.*, a.nama_alat 
                    FROM alat_tambah at 
                    ...
                    // Balik urutan data untuk memunculkan yang paling baru di atas
                    $query .= " ORDER BY at.id_alat_tambah DESC";
                    ...
                    if (mysqli_num_rows($result) > 0) {
                      ...
                      while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        <button class='btn $status_class btn-sm mt-3'" . ($status == 'Disetujui' ? "disabled" : "") . " onclick='EditStatus(
                            \"" . $row['id_alat_tambah'] . "\",
                            \"" . $row['id_alat'] . "\",
                            \"" . $row['status'] . "\",
                            \"" . $row['jumlah'] . "\"
                        )'>" . ($status ?: 'Diproses') . "</button>
                    </td>";
                        echo "<td class='align-middle text-center'>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      document.getElementById('tambahAlatPengadaan').addEventListener('submit', function(event) {
        ...
        xhr.open('POST', 'petugas/alat_tambah/alat_tambah.php', true);
        xhr.onload = function() {
          ...
          if (xhr.status === 200) {
            ...
            var responseData = response.split('|'); // Split respons untuk mendapatkan nama alat
            var responseStatus = responseData[0]; // Status respons
            var namaAlat = responseData[1]; // Nama alat

            if (responseStatus === 'success') {
              $('#modalAlatPengadaan').modal('hide');
              Swal.fire({
                title: "Berhasil!",
              ...
              Swal.fire("Error", "Data yang anda masukan belum lengkap", "error");
            } else if (response === 'Nama_alat_tidak_ditemukan') {
              Swal.fire("Error", "Nama alat tidak ditemukan", "error");
            } else {
              Swal.fire("Error", "Gagal menambahkan data alat", "error");
            }
          } else {
    ...
    function openEditModal(id_alat_tambah, id_alat, waktu_input, jumlah, deskripsi) {
      // Isi data ke dalam form edit
      document.getElementById('id_alat_tambah').value = id_alat_tambah;
      document.getElementById('editid_alat').value = id_alat;
      document.getElementById('editwaktu').value = waktu_input;
      document.getElementById('editjumlah').value = jumlah;
      document.getElementById('editdeskripsi').value = deskripsi;
      $('#editModalAlatPengadaan').modal('show');
    }

    // logika untuk mengedit data data pemimpin
    document.addEventListener('DOMContentLoaded', function() {
      document.getElementById('editAlatPengadaan').addEventListener('submit', function(event) {
        event.preventDefault(); // Menghentikan aksi default form submit
        ...
        var xhr = new XMLHttpRequest();
        xhr.open('POST', 'petugas/alat_tambah/alat_edit.php', true);
        xhr.onload = function() {
          // Sembunyikan elemen .loading setelah permintaan AJAX selesai
          loding.style.display = 'none';
          ...
          if (xhr.status === 200) {
            ...
            var namaAlat = responseData[1]; // Nama alat
            if (responseStatus === 'success') {
              $('#editModalAlatPengadaan').modal('hide');
              Swal.fire({
                title: "Berhasil!",
                text: namaAlat + " berhasil diedit",
                icon: "success",
                confirmButtonText: "OK"
              }).then((result) => {
                if (result.isConfirmed) {
              ...
              Swal.fire("Error", "Data alat sudah digunakan silakan masukan data alat lain", "error");
            } else {
              Swal.fire("Error", "Gagal menambahkan data alat", "error");
            }
          } else {
    ...
    function hapus(id_alat, id_alat_tambah) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          xhr.open('POST', 'petugas/alat_tambah/alat_hapus.php', true);
          ...
          xhr.onload = function() {
            ...
            if (xhr.status === 200) {
              var response = xhr.responseText;
              if (response === 'success') {
                Swal.fire("Sukses!", "Data alat berhasil dihapus.", "success")
                  .then(() => {
                    // Refresh halaman setelah penghapusan berhasil
                    window.location.reload();
                  });
              } else {
                Swal.fire("Error", "Gagal menghapus data alat.", "error");
              }
            } else {
              Swal.fire("Error", "Terjadi kesalahan saat mengirim data.", "error");
            }
          };
          xhr.send("id_alat=" + id_alat + "&id_alat_tambah=" + id_alat_tambah);
        } else {
    ...
    function EditStatus(id_alat_tambah, id_alat, status, jumlah) {
      Swal.fire({
        title: "Setujui Data ini?",
        text: "Setelah disetujui, Jumlah permintaan pengadaan alat akan ditambahkan ke data alat!",
        icon: "info",
        showCancelButton: true,
        confirmButtonText: "Batal",
        cancelButtonText: "Ya, Setujui!",
        confirmButtonColor: "#d33",
        cancelButtonColor: "#3085d6"
      }).then((result) => {
        if (result.isConfirmed) {
          // Jika pengguna membatalkan
          Swal.fire("Status Setujui dibatalkan", "", "info");
        } else {
          // Jika pengguna mengonfirmasi untuk menyetujui
          var xhr = new XMLHttpRequest();
          xhr.open('POST', 'admin/alat_tambah/alat_status.php', true);
          xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
          xhr.onload = function() {
            if (xhr.status === 200) {
              var response = xhr.responseText;
              ...
              if (responseStatus === 'success') {
                ...
                Swal.fire({
                  title: "Berhasil!",
                  text: "Data " + namaAlat + " berhasil disetujui",
                  icon: "success"
                }).then((value) => {
                  if (value) {
                    // Reload halaman setelah menekan tombol OK
                ...
                Swal.fire("Error", "Gagal menyetujui status alat.", "error");
              }
            } else {
              Swal.fire("Error", "Terjadi kesalahan saat mengirim data.", "error");
            }
          };
          xhr.send("id_alat_tambah=" + id_alat_tambah + "&id_alat=" + id_alat + "&status=" + status + "&jumlah=" + jumlah);
        }
      });
    }
    ...
    function selectOption(jumlah) {
      ...
      window.location.href = "petugas_data_alat_pengadaan?jumlah=" + jumlah;
    }
  </script>
  <script src="https://code.jquery.com/jquery-3.6.0.min.js" integrity="sha256-Km4P/A4C3CfpzPnqGk3sD4cxsMkSCGS2U5x8bC+v6j8=" crossorigin="anonymous"></script>
