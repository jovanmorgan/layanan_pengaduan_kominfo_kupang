...
if (empty($id_alat_tambah) || empty($id_alat) || empty($jumlah) || empty($deskripsi) || empty($waktu)) {
...
$query_jumlah_sebelumnya = "SELECT jumlah FROM alat_tambah WHERE id_alat_tambah = '$id_alat_tambah'";
...
if ($result_jumlah_sebelumnya) {
    ...
    if ($result_stok) {
        ...
        if ($stok_alat >= $jumlah) {
            // Buat query SQL untuk mengupdate data pada tabel alat_tambah
            $query_update_alat_tambah = "UPDATE alat_tambah SET id_alat = '$id_alat', jumlah = '$jumlah', deskripsi = '$deskripsi', waktu = '$waktu_formatted' WHERE id_alat_tambah = '$id_alat_tambah'";

            // Jalankan query
            if (mysqli_query($koneksi, $query_update_alat_tambah)) {
