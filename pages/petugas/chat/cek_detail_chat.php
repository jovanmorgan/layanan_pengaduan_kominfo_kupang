...
if (isset($_GET['id_topik_chat_petugas'])) {
    ...
    echo '<button class="btn btn-primary font-weight-bold text-xs" onclick="tambahChat(' . $id_topik_chat_petugas . ', \'' . $id_petugas . '\')">';
    ...
    if (mysqli_num_rows($result_detail_chat_petugas) > 0) {
        ...
        // Tampilkan data dalam format yang diinginkan
        echo '<br><button class="btn btn-primary font-weight-bold text-xs" onclick="tambahChat(' . $id_topik_chat_petugas . ', \'' . $id_petugas . '\')">';
        echo 'TAMBAH CHAT <i class="fas fa-comments text-white me-2" aria-hidden="true"></i>';
        echo '</button>';
    } else {
